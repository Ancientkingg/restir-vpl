...
void reservoir::update(triangular_light new_sample, point3 sample_point, int w_i) {
    W += w_i;
    M++;
    if (W == w_i) {
        sample = new_sample;
...
// reservoir class utility functions
reservoir merge_reservoirs(const std::vector<reservoir> &reservoirs) {
    reservoir merged;
...
// restir_light_sampler class implementation
void restir_light_sampler::set_initial_sample(u_int i, u_int j, ray &r) {
...
void restir_light_sampler::visibility_check(u_int i, u_int j, ray &r, tinybvh::BVH &bvh) {
...
void restir_light_sampler::temporal_update(u_int i, u_int j) {
...
void restir_light_sampler::spatial_update(u_int i, u_int j) {
...
void restir_light_sampler::spatial_update(u_int i, u_int j, int radius) {
    for (int r = 0; r < radius; r++) {
        spatial_update(i, j);
    }
}

void restir_light_sampler::next_frame() {
    // Swap the current and previous reservoirs
    std::swap(prev_reservoirs, current_reservoirs);
}

triangular_light restir_light_sampler::pick_light() {
...
int restir_light_sampler::get_light_weight(triangular_light &light, ray &r) {
